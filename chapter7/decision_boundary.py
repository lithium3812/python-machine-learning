import numpy as np
import matplotlib.pyplot as plt
from matplotlib.colors import ListedColormap
from mpl_toolkits.mplot3d import Axes3D


def plot_regions(X, y, classifier, test_idx=None, resolution=0.02):
    # marker generator and color map
    markers = ('s', 'x', 'o', '^', 'v')
    colors = ('red', 'blue', 'lightgreen', 'gray', 'cyan')
    cmap = ListedColormap(colors[:len(np.unique(y))])

    # plot the decision surface
    x1_min, x1_max = X[:,0].min() - 1, X[:,0].max() + 1
    x2_min, x2_max = X[:,1].min() - 1, X[:,1].max() + 1
    xx1, xx2 = np.meshgrid(np.arange(x1_min, x1_max, resolution), np.arange(x2_min, x2_max, resolution))
    Z = classifier.predict(np.array([xx1.ravel(), xx2.ravel()]).T)
    Z = Z.reshape(xx1.shape)
    plt.contourf(xx1, xx2, Z, alpha=0.2, cmap=cmap)
    plt.xlim(xx1.min(), xx1.max())
    plt.ylim(xx2.min(), xx2.max())

    # plot class examples
    for idx, cl in enumerate(np.unique(y)):
        plt.scatter(x=X[y==cl, 0], y=X[y==cl, 1],
        alpha=0.9,
        c=colors[idx], 
        marker=markers[idx], 
        label=cl,
        edgecolor='gray', s=20)

    # highlight test examples by circles
    if test_idx:
        X_test = X[test_idx, :]
        plt.scatter(x=X_test[:, 0], y=X_test[:, 1],
        alpha=1.0,
        facecolor='none',
        marker='o', 
        label='test data',
        edgecolor='black',
        linewidth=1)


def plot_regions3D(X, y, classifier, test_idx=None, resolution=0.02):
    # marker generator and color map
    markers = ('s', 'x', 'o', '^', 'v')
    colors = ('red', 'blue', 'lightgreen', 'gray', 'cyan')

    # plot the decision surface
    x1_min, x1_max = X[:,0].min() - 1, X[:,0].max() + 1
    x2_min, x2_max = X[:,1].min() - 1, X[:,1].max() + 1
    xx1, xx2 = np.meshgrid(np.arange(x1_min, x1_max, resolution), np.arange(x2_min, x2_max, resolution))
    coef = classifier.coef_[0]
    intercept = classifier.intercept_[0]
    Z = -(coef[0]*xx1+coef[1]*xx2+intercept)/coef[2]
    plt3d = plt.figure(figsize=(8, 6)).gca(projection='3d')
    ax = plt.gca()
    plt3d.plot_surface(xx1, xx2, Z, alpha=0.2)
    plt.xlim(xx1.min(), xx1.max())
    plt.ylim(xx2.min(), xx2.max())

    # plot class examples
    for idx, cl in enumerate(np.unique(y)):
        ax.scatter(X[y==cl, 0], X[y==cl, 1], X[y==cl, 2],
        alpha=0.9,
        c=colors[idx], 
        marker=markers[idx], 
        label=cl,
        edgecolor='gray', s=20)

    # highlight test examples by circles
    """
    if test_idx:
        X_test = X[test_idx, :]
        ax.scatter(X_test[:, 0], X_test[:, 1], X_test[:, 2],
        alpha=1.0,
        facecolor='none',
        marker='o', 
        label='test data',
        edgecolor='black',
        linewidth=1)
    """